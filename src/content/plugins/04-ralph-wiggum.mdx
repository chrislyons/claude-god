...
```bash
/ralph-loop "Scan all docs/ folders in carbon-acx, hotbox, orpheus-sdk, wordbird, helm, and undone.
...
```bash
/ralph-loop "Refactor orpheus-sdk/apps/clip-composer following patterns in ARCHITECTURE.md.
After EACH change:
1. Run tests with: cd orpheus-sdk && npm test
...
```bash
/ralph-loop "Implement user authentication in helm repository with TDD approach:
...
```bash
/ralph-loop "Apply spacing standards from ~/dev/docs/workspace-asset-standards.md to frontend CSS.
Repos to update: osd-v2, wordbird, listmaker, chrislyons-website, carbon-acx.
For each repo:
