...
<Callout type="info" title="Custom Skill">
**Note:** Ralph Wiggum is a custom skill that leverages Claude Code's Stop hook mechanism, not an official built-in plugin. It requires setup via `~/.claude/skills/` and `~/.claude/hooks.json`. The pattern described here is proven and powerful, but you'll need to implement it yourself.
</Callout>
