...
<Callout type="warning" title="Conceptual / Custom Implementation">
**Note:** Hookify is a conceptual pattern, not an official Claude Code plugin. The actual hooks system uses JSON configuration files (`~/.claude/hooks.json` or `<repo>/.claude/hooks.json`) rather than interactive commands. This document describes a custom skill that could be built on top of the hooks system.
</Callout>
